...
读取文件之前，使用 AskUserQuestion 工具询问词汇量等级。

**注意：该工具最多支持 4 个选项。** 按以下方式配置：

**question 字段内容：**
```
请选择你的英语水平（考研约8000词、雅思约8500+词，或自定义词汇量数字，请选「其他」手动输入）：
```
...
**用户通过「其他」手动输入时的解析规则：**
- 输入"考研"或"研究生" → 按 8000 词处理
- 输入"雅思"或"IELTS" → 按 8500 词处理
- 输入纯数字（如 2000、5000）→ 按该数字词汇量处理
- 其他自定义描述 → 根据描述判断最接近的词汇量等级
